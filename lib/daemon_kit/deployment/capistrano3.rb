...
puts "voce esta usando CAPIROTO 3"
...
_cset(:repo_url)  { abort "Please specify the repository that houses your application's code, set :repository, 'foo'" }
...
_cset(:revision)  { `git rev-parse HEAD` }
...
_cset(:shared_dir)        { set :shared_dir, "~/svc/#{fetch(:application)}/shared" }
...
_cset(:releases_path)     { File.join(fetch(:deploy_to), fetch(:version_dir, "")) }
_cset(:shared_path)       { File.join(fetch(:deploy_to), fetch(:shared_dir)) }
_cset(:current_path)      { File.join(fetch(:deploy_to), fetch(:current_dir)) }
_cset(:release_path)      { File.join(fetch(:releases_path), fetch(:release_name)) }
...
namespace :deploy do
  ...
  DESC
  task :setup do#, :except => { :no_release => true } do
  ...
  desc <<-DESC
    ...
    variable.
  DESC
  task :update_code do #, :except => { :no_release => true } do
  ...
  DESC
  task :finalize_update do #, :except => { :no_release => true } do
  ...
  DESC
  task :symlink do #, :except => { :no_release => true } do
    on_rollback do
      if previous_release
  ...
  DESC
  task :upload do #, :except => { :no_release => true } do
  ...
  desc <<-DESC
    ...
    additional info.
  DESC
  task :restart do #, :except => { :no_release => true } do
    stop
    start
  end
  ...
  namespace :rollback do
    ...
    DESC
    task :revision do #, :except => { :no_release => true } do
      if previous_release
    ...
    DESC
    task :cleanup do #, :except => { :no_release => true } do
    ...
    DESC
    task :code do #, :except => { :no_release => true } do
      revision
      cleanup
    end
  ...
  DESC
  task :check do #, :except => { :no_release => true } do
  ...
  namespace :pending do
    ...
    DESC
    task :diff do #, :except => { :no_release => true } do
    ...
    DESC
    task :default do #, :except => { :no_release => true } do
